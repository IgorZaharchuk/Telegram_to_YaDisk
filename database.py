...
class DatabaseManager:
    ...
    DEFAULT_COMPRESSION: Dict[str, Any] = {'min_photo_size_kb': 500, 'image_quality': 92, 'convert_heic': True, 'min_video_size_mb': 15, 'min_video_duration': 10, 'video_crf': 23, 'video_preset': 'veryfast', 'video_threads': 2, 'photo_processes': 4, 'skip_efficient_codecs': True, 'use_cpulimit': True, 'video_cpu_limit': 75, 'low_priority': True}
    ...
    async def _record_event(self, event_type: str, **kwargs: Any) -> None:
        ...
        if event_type == 'uploaded':
            ...
            await self.update_chat_stats(chat_id, {'uploaded': 1, 'uploaded_bytes': actual_size})
            await self.update_stage_stats('upload', {'processed': 1, 'success': 1})
        elif event_type == 'downloaded':
            await self.update_chat_stats(chat_id, {'total_bytes': kwargs.get('size', 0)})
            ...
            await self.update_chat_stats(chat_id, {'skipped': 1, 'total_bytes': kwargs.get('size', 0)})

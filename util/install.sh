...
# =============================================================================
        4)
            print_info "Режим 4: Настройка веб-интерфейса"
            setup_web_interface
            return
            ;;
...
choose_installation_mode() {
    ...
    echo "  4) Настройка веб-интерфейса"
    echo "  4) Настройка веб-интерфейса"
    echo ""
    read -p "Ваш выбор (1/2/3/4): " INSTALL_MODE
...
setup_web_interface() {
    ...
    cd "$HOME/Telegram_to_YaDisk" || return 1
    
    # Активируем venv
    source venv/bin/activate 2>/dev/null || { print_error "venv не найден!"; return 1; }
    
    # Установка Python-зависимостей
    print_step "Установка Python-зависимостей для веб-интерфейса..."
    ...
    # Конфиг Nginx
    read -p "Домен для веб-интерфейса (например, tg2ya.ru): " DOMAIN
    if [[ -n "$DOMAIN" ]]; then
        # Пароль для Basic Auth
        read -p "Логин для веб-интерфейса [admin]: " WEB_USER
...
NGINXEOF
        ...
        print_success "Nginx настроен: https://$DOMAIN/backup"
    fi
    
    # Systemd сервис
    print_step "Создание systemd сервиса для веб-интерфейса..."
...
EOWEB
    ...
    read -p "Включить и запустить веб-интерфейс сейчас? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        sudo systemctl enable tg2ya-web.service
        sudo systemctl start tg2ya-web.service
        sleep 2
        if sudo systemctl is-active --quiet tg2ya-web.service; then
            print_success "✅ Веб-интерфейс запущен"
        else
            print_warning "⚠️ Веб-интерфейс не запустился"
            sudo journalctl -u tg2ya-web --no-pager -n 10
        fi
    else
        print_info "Для запуска: sudo systemctl start tg2ya-web"
    fi
...
HTMLEOF
    print_success "Главная страница создана: https://$DOMAIN"
    ...
    echo ""
    echo "🔧 Управление:"
    echo "  sudo systemctl status tg2ya-web"
    echo "  sudo systemctl restart tg2ya-web"
    echo "  sudo journalctl -u tg2ya-web -f"
    echo ""
    echo "🌐 Доступ: https://$DOMAIN/backup"
    echo "🔑 Логин: $WEB_USER"
}
...
setup_web_interface() {
    ...
    if [[ ! -f "/etc/lighttpd/server.pem" ]]; then
        sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048             -keyout /etc/lighttpd/server.pem -out /etc/lighttpd/server.pem             -subj "/CN=$DOMAIN" 2>/dev/null
    fi
...
<body><h1>☁️ $DOMAIN</h1><p><a href="/backup">📊 Панель управления</a></p></body></html>
HTMLEOF
    print_success "Главная страница создана"
}
